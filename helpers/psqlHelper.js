...
const _testForDuplicateTxns = async (chainId, txnHash) => {
    try {
        ...
        const data = 
            await client.query(`SELECT COUNT(*) FROM ${tableDetails.name} 
...
const _getTxnByHash = async (chainId, txnHash) => {
    try {
        const tableDetails = psql.tables.txn(chainId);

        const data = 
            await client.query(`SELECT * FROM ${tableDetails.name} WHERE ${tableDetails.columns.txnHash.field}='${txnHash}';
                               `);

        return data.rows || [];
    } catch (err) {
        console.error(`Error getting txn by hash.`, err.message);
    }
}
