...
const { lastTrackedBlocks, addBlocksQueue, clearBlocksQueue, maintainProcessingQueue } = require("../cacheHelper");
...
const _produceBlockForChain = async (chainId) => {
  ...
  let nextTrackBlock = lastTrackedBlocks[chainId] ? parseInt(lastTrackedBlocks[chainId]) + 1 : undefined;
  ...
  try {
    ...
    for (let i = blockNumber - nextTrackBlock > 1000 ? blockNumber : nextTrackBlock; i <= blockNumber; i++) {
      blocks.push(`${i}`);
    }
